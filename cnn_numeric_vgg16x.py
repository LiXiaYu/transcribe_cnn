...
def main():
    py_filename = "cnn_numeric_vgg16x"
    ...
    model = DefineModel_numerics_VGG16x(x_train_numerics.shape[1], numclasses)
    ...
    print(model.summary())
    print("训练神经网络")
    metrics1 = Kc.Mertics_f1s_recalls_precisions(validation_data=(x_test_numerics, y_test))
    # metrics2 = Kc.Mertics_roc_auc(validation_data=([x_test_text, x_test_numeric], y_test))
    history = model.fit(x_train_numerics, y_train, batch_size=400, validation_data=(x_test_numerics, y_test), epochs=300,
                        callbacks=[metrics1])
...
def DefineModel_numerics_VGG16x(oneshape: int, numclasses: int) -> Model:

    numerics_input = Input(shape=(oneshape, 1), dtype=tf.float32, name='numerics_input')
    conv1D_3 = layers.Conv1D(32, 5, padding='same', activation=tf.nn.relu, input_shape=(1, oneshape), name='conv1D_3')(
        numerics_input)
    ...
    bn_1 = layers.BatchNormalization()(dropout_2)
    ...
    model = Model(inputs=[numerics_input], outputs=[dense_vgg_6_4])
