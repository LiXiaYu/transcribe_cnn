...
def Generate_TLCPD(data: pd.DataFrame)->pd.DataFrame:
    iop = data['Right_Intraocular_Pressure']
    ...
    x_train = []
    y_train = []
    # model = DefineModel_generate_TLCPD(x_train.shape[1])

    return data


def DefineModel_generate_TLCPD(oneshape: int) -> Model:
    numerics_input = Input(shape=(oneshape, 1), dtype=tf.float32, name='numerics_input')
    numerics_bn = layers.BatchNormalization()(numerics_input)
    conv_1 = layers.Conv1D(32, 5, padding='same', activation=tf.nn.relu, input_shape=(1, oneshape[0]))(numerics_bn)
    conv_2 = layers.Conv1D(64, 5, padding='same', activation=tf.nn.relu, name='conv1D_2')(conv_1)
    conv_3 = layers.Conv1D(64, 5, padding='same', activation=tf.nn.relu, name='conv1D_2')(conv_2)

    flatten_1 = layers.Flatten()(conv_3)
    dense_1 = layers.Dense(oneshape, activation=tf.nn.softmax)(flatten_1)
    model = Model(inputs=[numerics_input], outputs=[dense_1])
